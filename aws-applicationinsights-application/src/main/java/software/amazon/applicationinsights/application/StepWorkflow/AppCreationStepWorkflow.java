...
public class AppCreationStepWorkflow extends BaseStepWorkflow{
    ...
    protected boolean isCurrentItemProcessFinished(
            ...
            Logger logger) {
        return HandlerHelper.describeApplicationInsightsApplication(processingItem, proxy, applicationInsightsClient)
                .applicationInfo()
                .lifeCycle()
    ...
    protected void startProcessNextItem(
            ...
            ApplicationInsightsClient applicationInsightsClient,
            Logger logger) {
        // APP_CREATION step has no next item to process
    }
}
